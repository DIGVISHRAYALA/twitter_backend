...
app.get('/', (req, res) => {
  res.json({ success: true, msg: 'Node server running' });
});
...
app.post('/register', async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, 10);
    const profilePic = images[username[0]?.toUpperCase()] || defaultImage;
  ...
  try {
    ...
    if (email) {
      const exists = await User.findOne({
        where: { email, username: { [Op.ne]: username } }
      });
    ...
    if (profilePic) {
      user.profilePic = profilePic;
      await Post.update({ profilepic: profilePic }, { where: { username } });
    }
    ...
    res.json({ message: 'Editing user Successful' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Editing user Failed' });
  }
});
...
app.delete('/deleteUserFromDB', async (req, res) => {
  try {
    ...
    await Follow.destroy({ where: { [Op.or]: [{ username }, { following: username }] } });
    await Notification.destroy({ where: { username } });
    await PostLikes.destroy({ where: { username } });
    await PostTweets.destroy({ where: { username } });

    const posts = await Post.findAll({ where: { username } });
    for (const post of posts) {
      await PostLikes.destroy({ where: { postId: post.id } });
      await PostTweets.destroy({ where: { postId: post.id } });
    }

    await Post.destroy({ where: { username } });
...
app.post('/post', async (req, res) => {
  try {
    ...
    await User.increment('posts', { where: { username } });
    ...
    res.json({ message: 'Post saved successful' });
  } catch (err) {
...
app.post('/likeForAPost', async (req, res) => {
  ...
  const like = await PostLikes.findOne({ where: { postId, username } });

  if (like) {
    await PostLikes.destroy({ where: { postId, username } });
    await Post.decrement('totalLikes', { where: { id: postId } });
    return res.json({ message: 'Like removed' });
  }

  await PostLikes.create({ postId, username });
  await Post.increment('totalLikes', { where: { id: postId } });
...
app.post('/tweetToAPost', async (req, res) => {
  ...
  await PostTweets.create({
    postId,
    username,
    tweet: tweetContent
  });

  await Post.increment('totalTweets', { where: { id: postId } });

  res.json({ message: 'Tweet success' });
});
...
// =================================================
app.post('/tweetToEdit', async (req, res) => {
  const { postId, tweetContent } = req.body;
  await PostTweets.update({ tweet: tweetContent }, { where: { id: postId } });
...
app.post('/tweetToDelete', async (req, res) => {
  ...
  const tweet = await PostTweets.findByPk(postId);
  if (!tweet) return res.status(400).json({ message: 'Tweet not found' });

  await Post.decrement('totalTweets', { where: { id: tweet.postId } });
  await tweet.destroy();
...
app.post('/userPosts', async (req, res) => {
  ...
  for (const post of posts) {
    post.dataValues.tweets =
      await PostTweets.findAll({ where: { postId: post.id } });
  }

  res.json({ AllMyPosts: posts, user });
});
...
app.post('/FollowUser', async (req, res) => {
  ...
  await Follow.create({ username, following });
  await User.increment('following', { where: { username } });
  await User.increment('followers', { where: { username: following } });
...
app.post('/UnFollowUser', async (req, res) => {
  ...
  await Follow.destroy({ where: { username, following } });
  await User.decrement('following', { where: { username } });
  await User.decrement('followers', { where: { username: following } });

  res.json({ message: 'unfollowing success' });
});
...
app.post('/AllFollowingList', async (req, res) => {
  const { username } = req.body;
  const followingUsers = await Follow.findAll({ where: { username } });
...
app.post('/AllFollowersList', async (req, res) => {
  ...
  const followers = await Follow.findAll({ where: { following: username } });
  const following = await Follow.findAll({ where: { username } });

  const followingSet = new Set(following.map(f => f.following));

  const result = followers.map(f => ({
    ...f.dataValues,
    isFollowing: followingSet.has(f.username)
  }));

  res.json({ followersList: result });
});

// =================================================
// ALL USERS (NOT FOLLOWED)
// =================================================
app.post('/AllUsers', async (req, res) => {
  const { username } = req.body;
  ...
  const followed = following.map(f => f.following);

  const users = await User.findAll({
    where: {
      username: { [Op.notIn]: [...followed, username] }
    }
  });
